...
func TestPush(t *testing.T) {
	...
	scenarios := []struct {
		...
		},
		{
			...
			message: badRequest,
			queue:   queueName,
			err:     errors.New("Invalid message: Bad request"),
		},
		{
			name:    "success",
			mock:    mockAmazonSQS{resp: sqs.SendMessageOutput{MessageId: aws.String("messageId")}, queue: queueName, message: `{"MessageType":"REQ_UPDATED","RequestID":"1240358122","MatID":"SV11232-TX","Destination":"S3_Staging_Subtitles","Status":"Calculated"}`},
			message: "%BRONOT 'REQUEST','ip-10-238-252-171-Turbine','REQ_UPDATED','1240358122','SV11232-TX','S3_Staging_Subtitles','','Calculated','','ip-10-238-252-171'%",
			queue:   queueName,
			err:     nil,
		},
	}
